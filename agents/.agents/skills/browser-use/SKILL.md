...
This creates 5 executables: `browser`, `browser-use`, `browser-use-tui`, `browseruse`, `bu`

**Other systems:**
```bash
pipx install browser-use    # If pipx available
pip install browser-use     # With venv or --break-system-packages
```

**Python 3.14 Compatibility Note:**
The `browser-use doctor` command has a known asyncio issue on Python 3.14+. Core commands (`open`, `state`, `click`, etc.) work fine — the diagnostic just fails. Don't let the doctor error block you.
...
Falls back to probing ports 9222/9229 if no explicit CDP URL provided.
...
**Don't overcomplicate with Python subprocess.** The browser-use daemon handles display detection automatically. Just use the terminal tool directly:

```bash
# ✅ Correct — use terminal tool with browser-use directly
browser-use --headed open https://example.com
browser-use state
browser-use input 3 "some text"

# ❌ Wrong — don't use Python subprocess with DISPLAY env vars
# This is unnecessary and often fails in sandboxed environments
```

The `--headed` flag will launch a visible Chrome window when a display is available. The daemon manages the browser lifecycle automatically.
